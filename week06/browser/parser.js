 ...
 * @Description: parser
 * @LastEditors: Looper
 * @LastEditTime: 2020-05-20 23:48:10
 * @FilePath: /Frontend-01-Template/week06/browser/parser.js
 */
let css = require("css");
...
function match(element, selector) {
  ...
  for (s of combineSelectors) {
    ...
    if (s.charAt() == "#") {
      ...
      if (!attr || attr.value.split(" ").indexOf(s.replace(".", "")) === -1)
        return false;
    } else {
    ...
    if (matched) {
      ...
      for (let declaration of rule.declarations) {
        ...
        if (!computedStyle[declaration.property].specificity) {
          ...
          // 当前选择器优先级大于等于element的选择器优先级时进行样式覆盖
          computedStyle[declaration.property].value = declaration.value;
          computedStyle[declaration.property].specificity = sp;
        }
      }
